# -*- coding: utf-8 -*-
"""Advanced_API_Anomaly_Detection.ipynb
...
🛠️ Tools: AWS Lambda / Azure Functions / Cloud Functions + API Gateway + Retry & logging logic.
...
🛠️ Tools: PySpark (on EMR, Databricks, or GCP DataProc), Pandas + Scikit-Learn for preprocessing.
...
🛠️ Tools: AWS SageMaker / Azure ML / Vertex AI + MLflow for tracking + Dask/Ray for parallelization.
...
🛠️ Tools: MLflow + custom scoring logic or AutoML services if label data exists.
...
🛠️ Tools: Online inference via containerized API (Flask/FastAPI) or batch scoring in Spark/Databricks.
...
🛠️ Tools: FastAPI + Docker + API Gateway (with auth), or push via cloud-native messaging (SNS, Event Hub, Pub/Sub).
...
def select_best_model_by_silhouette(df,result_dict):
    ...
    if best_model:
        ...
        print("\n⚠️ No valid model found based on silhouette score.")
