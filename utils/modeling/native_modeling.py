...
def get_time_series_split(
    X: np.ndarray | pd.DataFrame, n_splits: int = 5
...
def calculate_clustering_score(
    data: np.ndarray | pd.DataFrame, labels: np.ndarray | pd.Series, metric: str
) -> float:
    if len(set(labels)) <= 1:
        return -1
...
def run_all_anomaly_detectors(
    df: pd.DataFrame, methods: List[str], eval_metric: str, complexity: str
    ...
    for method in methods:
        if method == "isolation_forest":
            ...
            for params in ParameterGrid(param_grid):
                ...
                score = calculate_clustering_score(
                    data=data, labels=preds, metric=eval_metric
                )
                if score > best_score:
                    best_score = score
                    best_model = model
