...
for c in numeric_cols:
    if use_spark:
        ingested_df = ingested_df.withColumn(c, col(c).cast(DoubleType()))
    else:
        ingested_df[c] = pd.to_numeric(
            ingested_df[c], errors="coerce"
        )  # converts and sets invalid parsing to NaN
